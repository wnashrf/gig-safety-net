...
function EmergencyBuilder({ onSave }: { onSave: (label: string) => void }) {
  ...
  const target = expenses * horizon;
  const saveRate = regular ? 0.15 : 0.12;
  const monthlySave = Math.max(50, Math.round((income * saveRate) / 10) * 10);
  const months = Math.max(1, Math.ceil(target / monthlySave));
  ...
  return (
    <div className="calculator-shell emergency-shell">
      <div className="calculator-intro"><div><SectionEyebrow icon={PiggyBank}>Dana kecemasan • Simpan untuk tenang</SectionEyebrow><h2>Kalau bulan depan<br /><em>tak seperti biasa?</em></h2><p>Bina kusyen kecil untuk minyak, sewa, makan dan komitmen asas — walaupun bulan pendapatan perlahan.</p></div><div className="source-chip blue-chip"><Wallet size={15} /> Manual dahulu</div></div>
      <div className="input-grid three"><label className="field"><span>Perbelanjaan asas bulanan</span><div className="input-wrap"><span>RM</span><input type="number" min={500} step={100} value={expenses} onChange={(e) => setExpenses(Number(e.target.value) || 0)} /></div></label><label className="field"><span>Pendapatan bulanan</span><div className="input-wrap"><span>RM</span><input type="number" min={500} step={100} value={income} onChange={(e) => setIncome(Number(e.target.value) || 0)} /></div></label><div className="field"><span>Pendapatan anda biasanya...</span><div className="segmented"><button className={regular ? "active" : ""} onClick={() => setRegular(true)}>Tetap</button><button className={!regular ? "active" : ""} onClick={() => setRegular(false)}>Berubah</button></div></div></div>
      ...
      <div className="emergency-result"><div className="target-number"><span className="muted-label">SASARAN ANDA</span><strong>{formatRM(target)}</strong><small>{horizon} bulan × {formatRM(expenses)} belanja asas</small></div><div className="save-number"><span className="muted-label">CADANGAN SIMPAN</span><strong>{formatRM(monthlySave)}<small> / bulan</small></strong><span className="save-rate"><TrendingUp size={14} /> {Math.round(saveRate * 100)}% pendapatan</span></div><div className="time-number"><span className="muted-label">ANGGARAN SIAP</span><strong>{months} <small>bulan</small></strong><span>Jika konsisten</span></div></div>
