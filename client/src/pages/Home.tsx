...
const productText = (value: string) => currentLanguage === "bm" ? value : ({
  "Perlindungan medikal": "Medical protection",
  "Kemalangan diri": "Personal accident",
  "Perlindungan rider": "Rider protection",
  "Medikal & hospital": "Medical & hospital",
  "Kemalangan & rider": "Accident & rider",
  "Semak PDS": "Check PDS",
  "Rujukan": "Reference",
}[value] ?? value);
...
function SocsoCalculator({ onSave, initialInputs, onSync }: { onSave: (label: string) => void; initialInputs: CalculatorInputs; onSync: SyncInputs }) {
  ...
  return (
    <div className="calculator-shell calculator-hover-card socso-hover-card">
      ...
      <div className="calculator-intro"><div><SectionEyebrow icon={ShieldCheck}>{localized("SOCSO • Lindung Kendiri", "SOCSO • Self-Employed Protection")}</SectionEyebrow><h2>{localized("Kalau berlaku apa-apa", "What if something happens")}<br /><em>{localized("masa bekerja?", "while working?")}</em></h2><p>{localized("SKSPS bantu lindungi anda daripada kemalangan kerja, penyakit pekerjaan, hilang upaya dan kematian.", "SKSPS helps protect you from work accidents, occupational illness, disability, and death.")}</p></div><div className="source-chip coral-chip"><BadgeCheck size={15} /> {localized("Rujukan PERKESO", "PERKESO reference")}</div></div>
      <div className="socso-callout"><div className="callout-icon"><ShieldCheck size={20} /></div><div><b>{localized("Ini bukan simpanan EPF.", "This is not EPF savings.")}</b><p>{localized("EPF bina duit untuk hari tua. Lindung Kendiri bantu bila anda cedera atau tak boleh bekerja.", "EPF builds money for retirement. Self-Employed Protection helps when you are injured or unable to work.")}</p></div></div>
      <div className="input-grid two"><label className="field"><span>{localized("Pendapatan bulanan untuk rujukan", "Monthly income for reference")}</span><div className="input-wrap"><span>RM</span><input type="number" min={1050} step={100} value={income} onChange={(e) => { const value = Number(e.target.value) || 0; setIncome(value); onSync({ socsoIncome: value }); }} /></div></label><label className="field"><span>{localized("Pilih tier pendapatan dilindungi", "Choose protected income tier")}</span><select className="select-field" value={selected} onChange={(e) => { const value = e.target.value; setSelected(value); onSync({ socsoTier: value }); }}>{socsoTiers.map((item) => <option key={item.label} value={item.label}>{item.label} / {localized("bulan", "month")}</option>)}</select></label></div>
      ...
      <div className="benefits-grid"><div className="benefit-card"><HeartPulse size={18} /><span>{localized("Rawatan perubatan", "Medical treatment")}</span><b>{localized("Disediakan", "Provided")}</b><small>{localized("Untuk kecederaan pekerjaan", "For work-related injuries")}</small></div><div className="benefit-card"><Wallet size={18} /><span>{localized("Hilang upaya sementara", "Temporary disability")}</span><b>{formatRM(tier.daily)} / hari</b><small>{localized("Indikatif ikut tier dipilih", "Indicative for the selected tier")}</small></div><div className="benefit-card"><UserRound size={18} /><span>{localized("Faedah tanggungan", "Dependent benefits")}</span><b>{tier.death}</b><small>{localized("Semak syarat rasmi", "Check official terms")}</small></div></div>
      <div className="result-foot socso-foot"><span><CircleHelp size={14} /> Premium & faedah adalah <b>indikatif</b>. Semak jadual rasmi semasa.</span><button className="button button-dark" onClick={() => onSave(`SKSPS ${formatRM(tier.premium, 2)}/tahun`)}>{localized("Simpan tier", "Save tier")} <Bookmark size={15} /></button></div>
...
function InsuranceComparison({ selected, setSelected, onSave }: { selected: string; setSelected: (id: string) => void; onSave: (label: string) => void }) {
  ...
  return (
    <div id="comparison" className="calculator-shell insurance-shell">
      <div className="calculator-intro"><div><SectionEyebrow icon={HeartPulse}>{localized("Insurans / Takaful • Banding dengan tenang", "Insurance / Takaful • Compare with confidence")}</SectionEyebrow><h2>{localized("Pilih perlindungan yang", "Choose protection that")}<br /><em>{localized("melengkapkan anda.", "complements your cover.")}</em></h2><p>{localized("Jika SKSPS sudah lindungi kemalangan kerja, lihat pilihan medikal yang boleh isi ruang kosong — bukan bayar dua kali.", "If SKSPS already covers work accidents, explore medical options that fill the gaps — without paying twice.")}</p></div><div className="source-chip mint-chip"><LockKeyhole size={15} /> {localized("Bukan ejen insurans", "Not an insurance agent")}</div></div>
      <div className="recommend-banner"><span className="recommend-star"><Sparkles size={16} /></span><div><b>{localized("Logik untuk anda", "A sensible fit")}</b><p>{localized("Mulakan dengan perlindungan medikal dahulu kerana SKSPS sudah fokus kepada kemalangan semasa bekerja.", "Start with medical protection because SKSPS already focuses on accidents at work.")}</p></div><span className="recommend-pill">{localized("Padanan terbaik", "Best match")}</span></div>
      <div className="comparison-controls"><div className="filter-group"><SlidersHorizontal size={14} /><span>{localized("Penapis:", "Filter:")}</span><button className={filter === "all" ? "active" : ""} onClick={() => setFilter("all")}>{localized("Semua", "All")}</button><button className={filter === "medical" ? "active" : ""} onClick={() => setFilter("medical")}>{localized("Fokus medikal", "Medical focus")}</button><button className={filter === "accident" ? "active" : ""} onClick={() => setFilter("accident")}>{localized("Kemalangan / rider", "Accident / rider")}</button></div><label className="sort-control"><ArrowDownUp size={14} /><span>{localized("Susun", "Sort")}</span><select value={sort} onChange={(e) => setSort(e.target.value)}><option value="coverage">{localized("Coverage paling luas", "Broadest coverage")}</option><option value="premium">{localized("Premium terendah", "Lowest premium")}</option></select></label></div><div className="comparison-table-wrap"><div className="comparison-labels"><span>{localized("Produk rujukan", "Reference products")}</span><span>Premium</span><span>{localized("Fokus perlindungan", "Protection focus")}</span><span>Hospital</span><span>{localized("Status", "Status")}</span><span /></div>{filteredProducts.map((product) => { const Icon = product.icon; const isSelected = selected === product.id; return <button key={product.id} className={`comparison-row ${isSelected ? "selected" : ""}`} onClick={() => setSelected(product.id)}><span className="product-cell"><span className={`product-icon ${product.tone}`}><Icon size={17} /></span><span><b>{product.name}</b><small>{productText(product.short)}</small></span></span><span className="table-value"><strong>RM {product.premiumValue}*</strong><small className="table-subnote">{localized("anggaran bulanan", "estimated monthly")}</small></span><span className="table-value">{productText(product.coverage)}</span><span className="table-value">{productText(product.hospital)}</span><span className="table-value"><span className={`status-pill ${isSelected ? "chosen" : "pending"}`}>{isSelected ? <><Check size={12} /> {localized("Dipilih", "Selected")}</> : productText(product.tag)}</span></span><span className="row-chevron"><ChevronRight size={17} /></span></button> })}</div>
      <div className="rubric-note"><CircleHelp size={15} /><p><b>{localized("Kenapa ada “semak”?", "Why does it say “check”? ")}</b> {localized("Harga contoh di atas hanyalah anggaran prototaip; jumlah perlindungan dan proses tuntutan berubah mengikut produk. Kami hanya tunjukkan maklumat yang perlu disahkan daripada quote / PDS semasa sebelum anda membuat keputusan.", "The prices above are prototype estimates; coverage amounts and claims processes vary by product. Verify details using the current quote / PDS before deciding.")}</p></div>
      <div className="insurance-actions"><button className="button button-dark" onClick={() => onSave(`Insurans: ${insuranceProducts.find((p) => p.id === selected)?.name}`)}>{localized("Simpan pilihan", "Save choice")} <Bookmark size={15} /></button><button className="text-link" onClick={() => openExternal("https://www.mycoverage.my/")}>{localized("Semak dengan penyedia", "Check with provider")} <ExternalLink size={15} /></button><span className="small-disclaimer"><LockKeyhole size={13} /> {localized("Kami bukan ejen / penasihat kewangan.", "We are not an agent / financial adviser.")}</span></div>
    </div>
  );
}
...
function EmergencyBuilder({ onSave, initialInputs, onSync }: { onSave: (label: string) => void; initialInputs: CalculatorInputs; onSync: SyncInputs }) {
  ...
  return (
    <div className="calculator-shell emergency-shell">
      <div className="calculator-intro"><div><SectionEyebrow icon={PiggyBank}>{localized("Dana kecemasan • Simpan untuk tenang", "Emergency fund • Save for peace of mind")}</SectionEyebrow><h2>{localized("Kalau bulan depan", "What if next month")}<br /><em>{localized("tak seperti biasa?", "is not usual?")}</em></h2><p>{localized("Bina kusyen kecil untuk minyak, sewa, makan dan komitmen asas — walaupun bulan pendapatan perlahan. Sasaran mudah: mula dengan 3 bulan perbelanjaan asas, kemudian tambah sedikit demi sedikit bila pendapatan lebih baik.", "Build a small cushion for fuel, rent, food, and essentials — even during a slow month. A simple target is three months of basic expenses, then add more when you can.")}</p></div><div className="source-chip blue-chip"><Wallet size={15} /> {localized("Manual dahulu", "Manual first")}</div></div><div className="emergency-guide"><div className="guide-heading"><span className="guide-kicker">{localized("PANDUAN LANGKAH DEMI LANGKAH", "STEP-BY-STEP GUIDE")}</span><b>{localized("Mulakan dengan jumlah yang boleh kekal.", "Start with an amount you can maintain.")}</b><small>{localized("Tak perlu tunggu ada lebihan besar — yang penting konsisten dan mudah dicapai bila perlu.", "You do not need a large surplus — consistency and easy access matter most.")}</small></div><div className="guide-steps"><div><span>01</span><p><b>{localized("Kira sasaran", "Set your target")}</b><small>{localized("Darab belanja asas bulanan dengan 3, 6 atau 12 bulan.", "Multiply basic monthly expenses by 3, 6, or 12 months.")}</small></p></div><div><span>02</span><p><b>{localized("Pilih amaun bulanan", "Choose a monthly amount")}</b><small>{localized("Gunakan cadangan di bawah atau mula dengan RM50–RM100 dahulu.", "Use the suggestion below or start with RM50–RM100.")}</small></p></div><div><span>03</span><p><b>{localized("Simpan di tempat cair", "Keep it accessible")}</b><small>{localized("Pilih akaun simpanan yang selamat dan mudah dikeluarkan tanpa risiko tinggi.", "Choose a safe savings account that is easy to access without high risk.")}</small></p></div></div></div>
      <div className="input-grid three"><label className="field"><span>{localized("Perbelanjaan asas bulanan", "Basic monthly expenses")}</span><div className="input-wrap"><span>RM</span><input type="number" min={500} step={100} value={expenses} onChange={(e) => { const value = Math.max(0, Number(e.target.value) || 0); setExpenses(value); onSync({ emergencyExpenses: value }); }} /></div></label><label className="field"><span>{localized("Pendapatan bulanan", "Monthly income")}</span><div className="input-wrap"><span>RM</span><input type="number" min={500} step={100} value={income} onChange={(e) => { const value = Math.max(0, Number(e.target.value) || 0); setIncome(value); onSync({ emergencyIncome: value }); }} /></div></label><div className="field"><span>{localized("Pendapatan anda biasanya...", "Your income is usually...")}</span><div className="segmented"><button type="button" className={regular ? "active" : ""} onClick={() => { setRegular(true); onSync({ emergencyRegular: true }); }}>{localized("Tetap", "Steady")}</button><button type="button" className={!regular ? "active" : ""} onClick={() => { setRegular(false); onSync({ emergencyRegular: false }); }}>{localized("Berubah", "Variable")}</button></div></div></div>
      <div className="horizon-block"><span>{localized("Berapa lama nak dilindungi?", "How long should you be covered?")}</span><div className="horizon-buttons">{[3, 6, 12].map((value) => <button key={value} className={horizon === value ? "active" : ""} onClick={() => { setHorizon(value); onSync({ emergencyHorizon: value }); }}>{value} {localized("bulan", "months")}{value === 6 && <b>{localized("Disaran", "Recommended")}</b>}</button>)}</div></div>
      <div className="emergency-result" aria-live="polite"><div className="target-number"><span className="muted-label">{localized("SASARAN ANDA", "YOUR TARGET")}</span><strong>{formatRM(target)}</strong><small>{horizon} {localized("bulan", "months")} × {formatRM(safeExpenses)} {localized("belanja asas", "basic expenses")}</small></div><div className="save-number"><span className="muted-label">{localized("CADANGAN SIMPAN", "SUGGESTED SAVING")}</span><strong>{formatRM(monthlySave)}<small>{localized(" / bulan", " / month")}</small></strong><span className="save-rate"><TrendingUp size={14} /> {Math.round(saveRate * 100)}% {localized("pendapatan", "of income")}</span></div><div className="time-number"><span className="muted-label">{localized("BAKI KE SASARAN", "REMAINING TO TARGET")}</span><strong>{monthsRemaining === 0 ? localized("Siap", "Complete") : `${monthsRemaining}`} <small>{monthsRemaining === 0 ? "" : localized("bulan lagi", "months remaining")}</small></strong><span>{monthsRemaining === 0 ? localized("Sasaran telah dicapai", "Target reached") : `${formatRM(remaining)} ${localized("lagi jika konsisten", "remaining at this rate")}`}</span></div></div>
      <div className="fund-tracker"><div className="tracker-head"><div><span className="muted-label">{localized("CHECK-IN SIMPANAN", "SAVINGS CHECK-IN")}</span><h3>{formatRM(progress)} <small>{localized("daripada", "of")} {formatRM(target)}</small></h3></div><span className="tracker-percent">{Math.round(progressPct)}%</span></div><div className="progress-track large"><span style={{ width: `${progressPct}%` }} /></div><div className="tracker-input"><label>{localized("Sudah berapa terkumpul?", "How much have you saved?")}</label><div className="input-wrap"><span>RM</span><input type="number" min={0} max={target} step={50} value={progress} onChange={(e) => { const value = Math.min(target, Math.max(0, Number(e.target.value) || 0)); setProgress(value); onSync({ emergencyProgress: value }); }} /></div><button className="button button-dark" onClick={() => { onSave(`Dana kecemasan ${formatRM(progress)} daripada ${formatRM(target)}`); toast.success(localized("Check-in disimpan pada peranti ini.", "Check-in saved on this device.")); }}>{localized("Simpan check-in", "Save check-in")} <Check size={15} /></button></div></div>
      <div className="vehicles-block"><div className="vehicles-title"><div><span className="muted-label">{localized("TEMPAT YANG MUDAH DICAPAI", "EASY-ACCESS PLACES")}</span><h3>{localized("Letak dana kecemasan di sini", "Keep your emergency fund here")}</h3></div><span className="tiny-note"><LockKeyhole size={13} />{localized(" Bukan pelaburan jangka panjang", " Not a long-term investment")}</span></div><div className="vehicles-grid">{vehicles.map(({ icon: Icon, name, tag, tone, pro, con }) => <div className="vehicle-card" key={name}><div className={`vehicle-icon ${tone}`}><Icon size={17} /></div><div className="vehicle-name"><b>{name}</b><span>{tag}</span></div><p><strong>+ </strong>{pro}<br /><strong>− </strong>{con}</p></div>)}</div></div>
    </div>
  );
}
