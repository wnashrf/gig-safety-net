...
function AccountProfileSummary({ profile, savedCount, completedPlanItems, hasCalculatorData }: { profile: { name: string; email: string; workerType: string }; savedCount: number; completedPlanItems: number; hasCalculatorData: boolean }) {
  return <div className="account-profile-summary"><div className="summary-avatar"><UserRound size={20} /></div><div className="summary-identity"><span className="muted-label">PROFIL AKAUN</span><h3>{profile.name || "Nama belum diisi"}</h3><p>{profile.email || "Emel akaun tidak tersedia"}</p></div><div className="summary-facts"><div><span>Jenis kerja</span><b>{profile.workerType || "Belum dipilih"}</b></div><div><span>Data kiraan</span><b>{hasCalculatorData ? "Ada simpanan" : "Belum disimpan"}</b></div><div><span>Plan</span><b>{completedPlanItems}/4 langkah</b></div></div></div>;
}
